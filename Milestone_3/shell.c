...
void launchShell() {
	...
	//while (1) {
		...
		//implementasi cd
			...
			//interrupt(0x21, 0x00 , "cmdAAA\0", 0x00 , 0x00);
			...
			if (buffer[2] == '\0') {
				...
				interrupt(0x21, 0x00 , "AAA", 0x00 , 0x00);
				//pindahkan ke directory yang baru
				changeDirectory(runprog, &result, curdir, &curdir);
				//interrupt(0x21, 0x20, curdir, argc, argv);
				if (result != 0) {
					interrupt(0x21,0x00, "Cannot find directory.\0", 0x00 ,0x00);
				} else if (result == 0) {
					interrupt(0x21, 0x20, curdir, argc, argv);
				}
			}
		...
		//KILL
			...
			interrupt(0x21, 0x0 , "Tidak ada command yang Anda maksud.\0" , 0x0 , 0x0);
			//interrupt(0x21, 0x0 , "\n" , 0x0 , 0x0);
			//interrupt(0x21, 0x0 , "\r" , 0x0 , 0x0);
